...
SELECT
  ...
  CASE
    ...
    WHEN ROUND(GREATEST(
            ...
            ABS(v1.p_drug       - v2.p_drug),
            ABS(v1.p_specialist - v2.p_specialist)
         ), 2) <= 0.20 THEN 'two-model-verified'
  ...
  CASE
    WHEN v1.p_bed IS NOT NULL AND v2.p_bed IS NOT NULL AND
         ROUND(GREATEST(ABS(v1.p_bed-v2.p_bed),ABS(v1.p_oxygen-v2.p_oxygen),ABS(v1.p_drug-v2.p_drug),ABS(v1.p_specialist-v2.p_specialist)), 2) <= 0.20
