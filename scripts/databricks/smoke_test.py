...
def q(name, sql):
    ...
    try:
        ...
        msg = f"databricks CLI returned non-JSON (rc={r.returncode}): {(r.stderr or r.stdout)[:200]}"
        print(f"  \033[1;31mFAILED:\033[0m {msg}")
        _FAILURES.append((name, msg))
        return False
    if d.get("status",{}).get("state") == "FAILED":
        msg = d['status'].get('error',{}).get('message')
        print(f"  \033[1;31mFAILED:\033[0m {msg}")
        _FAILURES.append((name, msg))
        return False
...
def main():
    ...
    if _FAILURES:
        print(f"\n\033[1;31m✗ smoke test FAILED — {len(_FAILURES)} queries failed\033[0m  ({elapsed:.1f}s)")
        for name, msg in _FAILURES:
            print(f"  - {name}: {msg}")
        sys.exit(1)
    print(f"\n\033[1;32m✓ smoke test complete\033[0m  ({elapsed:.1f}s)")
